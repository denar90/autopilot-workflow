...
# Create (or resume) an isolated worktree for plan-file mode and seed it. Mirrors
# phase01_worktree minus the Linear fetch. Sets $WT. Git/network dependent, so —
# like phase01_worktree — it is exercised end-to-end rather than unit-tested; the
# pure pieces (plan_slug, plan_branch_name, plan_seed_worktree) carry the coverage.
plan_setup_worktree() {
  local plan_src="$1" source_repo="$2"
  local slug branch project wt base
  slug=$(plan_slug "$plan_src")
  branch=$(plan_branch_name "$slug")
